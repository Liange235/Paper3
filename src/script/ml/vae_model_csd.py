...
class Encoder(nn.Module):
    def __init__(self, z_dim, hidden_dim, Md):
        super().__init__()
        # setup the three linear transformations used
        # self.cv1 = nn.Conv1d(132,hidden_dim,4)
        self.fc1 = nn.Linear(Md, hidden_dim)
...
class VAE(nn.Module):
    ...
    def __init__(self, z_dim=15, hidden_dim=67, Md=132):
        super().__init__()
        # create the encoder and decoder networks
        self.encoder = Encoder(z_dim, hidden_dim, Md)
        self.decoder = Decoder(z_dim, hidden_dim, Md)
        self.z_dim = z_dim
        self.Md = Md
...
def train(vae, y_train, load_model=0, dir=None, its=5000):
    if load_model:
        ...
        for epoch in range(its):
            epoch_loss = 0.0
            train_loader = setup_data_loaders(y_train, batch_size=500)
