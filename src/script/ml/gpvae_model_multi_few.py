...
def choose_kernel(k_id, ts):
    ...
    m_dict = {
        0: defaul_k,
        1: gpytorch.kernels.SpectralMixtureKernel(ard_num_dims=ts, num_mixtures=10,
                                                batch_size=[]),
        ...
        3: gpytorch.kernels.PiecewisePolynomialKernel(q=3, ard_num_dims=ts, active_dims=tuple(np.arange(ts))),
        4: gpytorch.kernels.PolynomialKernel(power=2, ard_num_dims=ts, active_dims=tuple(np.arange(ts))),
        5: gpytorch.kernels.RQKernel(ard_num_dims=ts, active_dims=tuple(np.arange(ts))),
        6: gpytorch.kernels.PeriodicKernel(num_dims=ts, active_dims=tuple(np.arange(ts)), period_length_prior=GammaPrior(2.0, 8.0))
    }
...
class MultitaskGPModel(gpytorch.models.ExactGP):
    def __init__(self, train_x, train_y, likelihood, ts, feature_augument):
        ...
        rq = choose_kernel(5, ts)
        pe = choose_kernel(6, ts)
        ...
        self.covar_module = gpytorch.kernels.MultitaskKernel(ker_custom, num_tasks=ts, rank=1, batch_size=[])
...
class Feature_Augument(torch.nn.Sequential):
    def __init__(self, data_dim=6, aug_feature1=10):
        ...
        self.add_module('linear2', torch.nn.Linear(aug_feature1, 6))
        self.add_module('acfun2', torch.nn.Sigmoid())
